...
class widefieldStack:
    def __init__(
        self, dir=None, numBaseline=30, startResponse=40, endResponse=45, divideFrame=1
    ) -> None:
        self.dir = dir
    ...
    def unmix(self, framesAfter=100, forceReload=False, use_dfof=False):
        ...
        if self.soundData is not None:
            ...
            if (
                (self.dir / "data_unmixed.mat").is_file()
                or (self.dir / "widefield.pkl").is_file()
            ) and not forceReload:
                ...
                for freq in range(nFreqs):
                    for atten in range(self.nAttens):
                        index = np.logical_and(
                            soundData["stimPlayedID"] == freqID[freq],
                            soundData["stimPlayedAtten"] == attens[atten],
                        )
                        ...
                        for repeat in range(nRep):
                            self.unmixed[:, :, :, freq, atten, repeat] = self.stack[
                                startFrames[repeat] : startFrames[repeat]
                                + framesToAnalyze,
                                :,
                                :,
                            ]
    ...
    def anomalies(self, std_thr=2):
        ...
        diffs = np.square(
            unmixed - np.repeat(np.expand_dims(mean_bl, axis=-1), nRep, axis=-1)
        )
    ...
    def generateMap(
        self,
        frameToTake=25,
        freqToTake=np.array([4, 2, 0]),
        save=True,
        save_mean=False,
        save_dir="maps2",
        percentile_norm=99.9,
        raw_multiplier=10,
    ):
        if self.unmixed is not None:
            ...
            blsubt = map - bl
            prctiles = np.percentile(
                blsubt[frameToTake, :, :, :, :], q=percentile_norm, axis=(0, 1)
            )
            ...
            for i in range(dfof.shape[-1]):
                io.imsave(
                    self.dir / save_dir / (str(i) + ".png"),
                    np.uint8(
                        filters.gaussian(norm[frameToTake, :, :, :, i], 0.5) * 255
                    ),
                )
                io.imsave(
                    self.dir / save_dir / (str(i) + "_raw.png"),
                    np.uint8(
                        filters.gaussian(dfof[frameToTake, :, :, :, i], 0.5) * 255
                    ),
                )

            if save_mean:
                io.imsave(
                    self.dir / save_dir / ("mean.tif"), np.mean(self.stack, axis=0)
                )
        else:
    ...
    def generateFRA(
        ...
        for freq in range(self.nStims):
            concat_atten = []
            for atten in range(self.nAttens):
                mean_freq_atten = np.nanmean(
                    self.unmixed[:, :, :, freq, atten, :], axis=-1
                )
                Fo = mean_freq_atten[0:numBaseline,].mean(axis=0)
                dfof = np.nanmean(
                    np.divide(mean_freq_atten - Fo, Fo)[startFrame:endFrame, :], axis=0
                )
                if atten == 0:
                    concat_atten = dfof
                else:
        ...
        if save:
            (self.dir / save_dir).mkdir(exist_ok=True)
            plt.imsave(
                self.dir / save_dir / "FRA.png", wholecat, vmin=0, vmax=vmax, cmap=cmap
            )

    def generateFRAmovie(
        self, save_dir="maps2", filename="FRA.mp4", max_norm=0.1, mode="dfof"
    ):
        ...
        for freq in range(self.nStims):
            ...
            for atten in range(self.nAttens):
                ...
                if atten == 0:
                    ...
                    concat_atten = np.concatenate(
                        (concat_atten, mean_freq_atten), axis=1
                    )
        ...
        out = cv2.VideoWriter(
            str(self.dir / save_dir / filename),
    ...
    def calculate_percentiles(self, percentile=95):
        ...
        stack_dfof_response = np.nanmean(
            stack_dfof[self.startResponse : self.endResponse, :], axis=-1
        )
        stack_dfof_response = np.max(stack_dfof_response, axis=0)

        percentiles = np.percentile(
            np.abs(stack_dfof_response), q=percentile, axis=(0, 1)
        )
        mask = np.abs(stack_dfof_response) > percentiles
        mask = mask.astype(float)
        mask[mask == False] = np.NaN
        percentileResponse = stack_dfof * np.repeat(
            np.expand_dims(mask, axis=-1), repeats=stack_dfof.shape[-1], axis=-1
        )
    ...
    def calculate_thresholds(self, startResponse=44, endResponse=46, p_crit=0.05):
        ...
        for freq in range(self.nStims):
            for atten in range(self.nAttens):
                baseline = self.percentileResponse[
                    : self.numBaseline, freq, atten
                ].mean(axis=0)
                stimulus = self.percentileResponse[
                    startResponse:endResponse, freq, atten
                ].mean(axis=0)
                p = scipy.stats.ttest_rel(
                    baseline, stimulus, nan_policy="omit", alternative="less"
                ).pvalue
                if p > 0.5:
                    break
                elif p < p_crit:
...
class wfstack2:
    ...
    def run_pipeline(self, save_dir="maps5"):
        ...
        (dfof, dfof_sm, baseline) = self.dfof(
            np.reshape(self.stack, (length, -1)).T,
            window=3000,
            step=1000,
            quantile=50,
            filter_fs=30,
        ...
        if self.unmixed is not None:
            ...
            for i in range(dfof.shape[-1]):
                ...
                io.imsave(
                    self.dir / save_dir / (str(i) + "_raw.png"),
                    np.uint8(
                        filters.gaussian_filter(dfof[frameToTake, :, :, :, i], 0.5)
                        * 255
                    ),
                )
        else:
    ...
    def soundData_to_pandas(self):
        ...
        soundData = pd.DataFrame(
            data=(
                {
                    "stimID": self.soundData["stimPlayedID"],
                    "stimAtten": self.soundData["stimPlayedAtten"],
                    "stimOnFrame": self.soundData["stimOnFrame"],
                    "trialNumber": 0,
                    "useTrial": False,
                }
            )
        )
        soundData = soundData.astype(
            {"stimID": np.int16, "stimAtten": np.int16, "stimOnFrame": np.int16}
        )

        for stim in soundData.stimID.unique():
            for atten in soundData.stimAtten.unique():
                soundData.loc[
                    (soundData["stimID"] == stim) & (soundData["stimAtten"] == atten),
                    "trialNumber",
                ] = np.arange(self.soundData["nRep"].astype(np.int16))
        ...
        for percentile in percentiles:
            for freqID, freq in enumerate(sorted(soundData.stimID.unique())):
                for attenID, atten in enumerate(sorted(soundData.stimAtten.unique())):
                    mask = (soundData["stimID"] == freq) & (
                        soundData["stimAtten"] == atten
                    )
                    mean_freq_atten = np.mean(
                        self.unmixed[
                            startResponse:endResponse, :, :, freqID, attenID, :
                        ],
                        axis=(0, -1),
                    )
                    percentiles = np.percentile(
                        np.abs(mean_freq_atten), q=percentile, axis=(0, 1)
                    )
                    mask2 = np.abs(mean_freq_atten) > percentiles
                    totalPixels = (
                        mask2.shape[0] * mask2.shape[1] * (100 - percentile) / 100
                    )
                    percentilesResponse[:, freqID, attenID] = (
                        self.unmixed[:, :, :, freqID, attenID, :]
                        * np.repeat(mask2[:, :, np.newaxis], repeats=10, axis=-1)
                    ).sum(axis=(1, 2)) / totalPixels
    ...
    def calculate_area(self, thresholdStd=2):
        ...
        signal = (
            stack_dfof > threshold[np.newaxis, :, :, np.newaxis, np.newaxis, np.newaxis]
        )
        self.area = signal
        ...
        def baseline(F):
            ...
            for i in np.arange(0, length - window, step):
                ...
                baseline_vals.append(
                    np.nanpercentile(temp[:, i : i + window], quantile, axis=1)
                )
